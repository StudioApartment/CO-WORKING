/* Shared public-text filter — used on the server and in The Office UI.
 * Names, full names, and social handles are public in the office, on badges,
 * and in email, so keep them clean and not spam. */

export const NAME_BLOCKED_MESSAGE = 'That name is not allowed — keep it friendly for the office.';
export const NAME_SPAM_MESSAGE = 'That looks like spam — use a real name.';
...
const TOKEN_BLOCK = new Set([
  ...
  'kike', 'kkk', 'kunt', 'milf', 'molester', 'motherfucker', 'nazi', 'negro', 'nigga',
...
export function isCleanName(raw) {
  return !publicNameError(raw);
}

export function publicNameError(raw) {
  return nameLanguageError(raw) || nameSpamError(raw);
}
...
/* URLs, keyboard-smash, and run-on junk in a 14–40 character field. Keeps
   ordinary short names (Li, Jo, Ana) and doubled letters (Lloyd, Aaron). */
export function nameSpamError(raw) {
  const name = String(raw || '').trim();
  if (!name) return null;
  if (/https?:\/\/|www\.|\.(com|net|org|io|xyz)\b/i.test(name)) return NAME_SPAM_MESSAGE;
  if (/(.)\1{4,}/.test(name)) return NAME_SPAM_MESSAGE;
  const letters = name.replace(/[^a-zA-Z]/g, '');
  if (name.length >= 6 && letters.length < 3) return NAME_SPAM_MESSAGE;
  return null;
}

export function dnaPublicError(dna) {
  if (!dna || typeof dna !== 'object') return null;
  const full = publicNameError(dna.fullName);
  if (full) return full;
  const socials = dna.socials;
  if (socials && typeof socials === 'object') {
    for (const v of Object.values(socials)) {
      if (v == null || v === '') continue;
      const err = publicNameError(String(v));
      if (err) return err;
    }
  }
  return null;
