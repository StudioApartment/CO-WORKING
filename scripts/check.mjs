...
for (const page of ['index.html', 'mii.html', 'admin.html']) {
  try {
...

{
  ...
  const wired = ['id="btnLost"', 'id="helpDock"', 'Lost my Co-Worker', 'openRecover()',
                 ...
                 'this.showRecover(\'\')',
                 'function hasActiveMii()', 'lost.hidden = has'].every((s) => src.includes(s));
  ...
  if (!wired || usesCamera) {
    note('✗', 'index.html — Lost my Co-Worker should open email recovery without the camera, and hide when a Co-Worker is active');
    failures++;
  } else {
    note('✓', 'index.html has a Lost my Co-Worker control that emails a reclaim link (hidden with an active Co-Worker)');
  }
}

{
  ...
  if (!wired) {
    note('✗', 'index.html — with an active Co-Worker, Join should hide and the name chip should offer edit/delete');
    failures++;
  } else {
    note('✓', 'index.html hides Join after claim and puts edit/delete on the name chip');
  }
}

{
  ...
  if (!src.includes("Customize Your Co-Worker")
    ...
    note('✗', 'index.html — after a photo the card should say Customize Your Co-Worker');
    failures++;
  } else {
    note('✓', 'index.html titles the post-photo card Customize Your Co-Worker');
  }
}

{
  ...
  if (!wired) {
    note('✗', 'index.html — Co-Workers need a city/ZIP location field with autocomplete');
    failures++;
  } else {
    note('✓', 'index.html asks for a city or ZIP and pins Gage / James Acklin');
  }
}

{
  ...
  if (!wired || src.includes('id="miiUrl"') || src.includes('id="camSocialRow"')
    ...
    note('✗', 'index.html — hovering a Co-Worker should open an info card (no website/socials in creation)');
    failures++;
  } else {
    note('✓', 'index.html shows a Co-Worker card on the info icon');
  }
}

{
  ...
  if (!wired) {
    note('✗', 'index.html — The Office should boot behind the index loader with the black lockup');
    failures++;
  } else {
    note('✓', 'index.html uses the index loader and black CO—WORKING lockup');
  }
}

/* A doubled comma in an object literal (`),` then another `,`) is a
 * SyntaxError in the browser. The Office then boots with Co-Workers 0 and a
 ...
 * unclosed brace from a bad merge — HTML script-tag balance does not. */
{
  const src = readFileSync(join(ROOT, 'index.html'), 'utf8');
  if (/\)\s*,\s*,/.test(src)) {
    note('✗', 'index.html — stray comma in an object literal will crash The Office');
    failures++;
  } else {
    note('✓', 'index.html object literals do not have a doubled comma');
  }
  const open = src.indexOf('<script type="module">');
  const close = src.lastIndexOf('</script>');
  if (open < 0 || close < open) {
    note('✗', 'index.html — could not extract the plaza module');
    failures++;
  } else {
    ...
    if (out.status !== 0) {
      ...
      note('✗', `index.html module does not parse — ${err || 'node --check failed'}`);
      failures++;
    } else {
      note('✓', 'index.html plaza module parses');
    }
  }
}
 ...
 * functions are healthy. The client must pin every route at /api/. */
{
  ...
  if (src.includes("fetch('api/") || src.includes('fetch("api/')) {
    note('✗', 'index.html — relative api/ fetch would 404 from a nested path');
    failures++;
  } else if (!src.includes("fetch('/api/config'") || !src.includes("expectsCloud()")) {
    note('✗', 'index.html — missing root-absolute /api/config fetch or expectsCloud()');
    failures++;
  } else if (!src.includes("location.hostname === 'coworking.fyi'")) {
    note('✗', 'index.html — missing apex→www bounce (phones otherwise see the office as offline)');
    failures++;
  } else {
    const listBlock = src.match(/async list\(\) \{[\s\S]*?\n  \},\n\n  \/\*\*/);
    if (!listBlock) {
      note('✗', 'index.html — could not find Store.list()');
      failures++;
    } else if (listBlock[0].includes('_head(') || !listBlock[0].includes("credentials: 'omit'")) {
      note('✗', 'index.html — list() must be a simple GET (credentials omit, no x-token)');
      failures++;
    } else {
      note('✓', 'index.html pins API calls at /api/ and keys claiming off Config');
    }
  }
}

{
  ...
  if (!src.includes("key: 'hat', label: 'Hat'") || !src.includes("key: 'apparel', label: 'Top'")) {
    note('✗', 'index.html — tray should say Hat and Top, not Headwear / Outfit');
    failures++;
  } else if (!src.includes("closest('#camPalette')")) {
    note('✗', 'index.html — colour palette must close when tapping outside it');
    failures++;
  } else {
    note('✓', 'index.html labels Hat / Top and dismisses the colour overlay');
  }
}

{
  ...
  if (!accept) {
    note('✗', 'index.html — could not find Cam.accept() → showBadge');
    failures++;
  } else if (/if\s*\(\s*res\.local\s*\)/.test(accept[0]) && accept[0].includes('this.close()')) {
    note('✗', 'index.html — a local save must still show the badge card');
    failures++;
  } else {
    note('✓', 'index.html shows the badge card after a local or server claim');
  }
}

{
  ...
  if (!pass) {
    note('✗', 'index.html — badge preview should be an Apple Wallet–style pass with dual Co-Worker + QR');
    failures++;
  } else {
    note('✓', 'index.html badge preview is a Wallet-style pass with orbit Co-Worker and QR');
  }
}

{
  ...
  if (!src.includes('function trayIsOff') || !src.includes('cyc-clear')) {
    note('✗', 'index.html — style tray should step linearly and offer a clear control');
    failures++;
  } else if (!src.includes('(i + 1) % options.length')) {
    note('✗', 'index.html — style tray should cycle options in order');
    failures++;
  } else if (ownBox || !sharedBox) {
    note('✗', 'index.html — the × should sit in the category pill, not in its own box');
    failures++;
  } else {
    note('✓', 'index.html style tray steps in order with a clear button');
  }
}

{
  ...
  if (!src.includes("geoCache(`afroO${r}`") || src.includes("add(G.ball(), 0, 0.74, -0.08")
    ...
    note('✗', 'index.html — afro should be a round cloudy puff with open forehead, not a V curtain');
    failures++;
  } else {
    note('✓', 'index.html afro is a round cloudy puff with open forehead');
  }
}

{
  ...
  if (!src.includes("sculptHair(`shcurlD${r}`") || !src.includes("'shortcurl'")) {
    note('✗', 'index.html — short curly hair style is missing');
    failures++;
  } else {
    note('✓', 'index.html includes short curly hair');
  }
}

{
  ...
  if (!src.includes("st === 'bald'") || src.includes("st === 'bald'") && /st === 'bald'[\s\S]{0,80}hairBand/.test(src)) {
    note('✗', 'index.html — bald should render with no hair geometry');
    failures++;
  } else {
    note('✓', 'index.html bald is fully hairless');
  }
}

{
  ...
  if (!wired || !furryCo || !noSage) {
    note('✗', 'index.html — furry bucket needs pile shells, a Co- mark, and furrysage aliased away');
    failures++;
  } else {
    note('✓', 'index.html includes furry bucket with pile and Co- mark');
  }
}

{
  ...
  const wired = ["label: 'Cat-eye'", "frame: 'cateye'", "label: 'Hex'",
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — loud plastic frames need cat-eye, hex, jelly and colourblock');
    failures++;
  } else {
    note('✓', 'index.html offers loud stylish plastic frames');
  }
}

{
  ...
  const wired = ["label: 'Tortoise pilot'", "frame: 'pilot'", 'browBar: true',
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — tortoise pilot needs a double bridge and a patterned rim');
    failures++;
  } else {
    note('✓', 'index.html offers a tortoiseshell plastic pilot frame');
  }
}

{
  ...
  if (!specs.every((s) => src.includes(s)) || !shape.every((s) => src.includes(s))) {
    note('✗', 'index.html — wrap shades should be a big chunky shield with a nose cutout');
    failures++;
  } else {
    note('✓', 'index.html wrap shades are a big chunky Pit Viper-style shield');
  }
}

{
  ...
  const wired = ["label: 'Big frames'", "frame: 'bold'", 'brow: 2.0',
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — big frames need a bold lens shape and a heavier browline');
    failures++;
  } else {
    note('✓', 'index.html offers big bold-framed glasses');
  }
}

{
  ...
  if (!layered) {
    note('✗', 'index.html — eyewear should be a post-hair overlay (not buried in the face shell)');
    failures++;
  } else {
    note('✓', 'index.html layers eyewear above hair');
  }
}

{
  ...
  const wired = ["label: 'Tri shades'", "frame: 'wedge'", "case 'wedge': {",
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — Tri shades need tiny triangular wedge lenses');
    failures++;
  } else {
    note('✓', 'index.html offers tiny triangular Italian-style shades');
  }
}

{
  ...
  if (listed || draws || !aliased) {
    note('✗', 'index.html — sideburns and mutton chops should be removed and aliased to none');
    failures++;
  } else {
    note('✓', 'index.html drops the sideburns and mutton chops options');
  }
}

{
  ...
  if (!wired || !creamScript || creamCo || !creamTone) {
    note('✗', 'index.html — club cap needs Co-Working script + horse-and-rider on a two-tone white/green band');
    failures++;
  } else {
    note('✓', 'index.html includes a two-tone white & green club cap with horse-and-rider mark');
  }
}

{
  ...
  if (!src.includes('makeBananaPatch') || !src.includes('H.banana') || !dadBanana || dadCo) {
    note('✗', 'index.html — dad hat needs a small banana patch, not a Co- mark');
    failures++;
  } else {
    note('✓', 'index.html puts a small banana on the dad hat');
  }
}

{
  ...
  if (!wired || /st === 'cowboy'\)/.test(src)) {
    note('✗', 'index.html — cowboy hat needs a creased cattleman crown and a swept brim');
    failures++;
  } else {
    note('✓', 'index.html shapes the cowboy hat with a cattleman crease');
  }
}

{
  ...
  if (!wired || !named) {
    note('✗', 'index.html — conical hat needs a plaited straw weave under a descriptive label');
    failures++;
  } else if (/coolie/i.test(src)) {
    note('✗', 'index.html — the conical hat must not be labelled with the slur');
    failures++;
  } else {
    note('✓', 'index.html includes a woven conical straw hat');
  }
}

{
  ...
  if (!wired || !oneFossil) {
    note('✗', 'index.html — one fossil beanie (black), jacquard + rib cuff, fossilblack aliased');
    failures++;
  } else {
    note('✓', 'index.html includes one jacquard fossil knit beanie');
  }
}

{
  ...
  if (!src.includes("geoCache(`mopB${r}`") || !src.includes('G.medium(1.07)')
    ...
    note('✗', 'index.html — bowl should render as a mop-top with tapered edges');
    failures++;
  } else {
    note('✓', 'index.html bowl is a mop-top with tapered edges');
  }
}

{
  ...
  if (!wired || shellOnDreads || tubes || bangsStubs || src.includes('locFade:')) {
    note('✗', 'index.html — locs should be centre-part face frames, not bangs or a fade matte');
    failures++;
  } else {
    note('✓', 'index.html locs are centre-part face-frame columns (no bangs)');
  }
}

{
  ...
  if (!shells.every((s) => src.includes(s))) {
    note('✗', 'index.html — barbershop cuts should be one photo-matched shell each');
    failures++;
  } else if (extras.some((s) => src.includes(s))) {
    note('✗', 'index.html — crop/pixie still attach extra balls that read as buns');
    failures++;
  } else {
    note('✓', 'index.html barbershop cuts are single photo-matched shells');
  }
}

{
  ...
  if (!src.includes("sculptHair(`combD${r}_${kind}`") || !src.includes("G.partedTop(1.05, 'combover')")
    ...
    note('✗', 'index.html — comb over should be a diagonal fringe with ears out and a jagged nape');
    failures++;
  } else {
    note('✓', 'index.html comb over has a diagonal fringe, ears out, and a jagged nape');
  }
}

{
  ...
  if (!src.includes("sculptHair(`bobF${r}`") || !src.includes('center-part chunks flowing')
    ...
    note('✗', 'index.html — bob should have center-part chunks and a low nape fill');
    failures++;
  } else {
    note('✓', 'index.html bob has center-part chunks and a low nape fill');
  }
}

{
  ...
  if (!src.includes("sculptHair(`shagI${r}`") || !src.includes('Chunky layered shag')
    ...
    note('✗', 'index.html — shag should have chunky layers and a jagged hem');
    failures++;
  } else {
    note('✓', 'index.html shag has chunky layers and a jagged hem');
  }
}

{
  ...
  if (!src.includes("sculptHair(`hitopH${r}`") || !src.includes("wolf: 'Hi-top'")
    ...
    note('✗', 'index.html — wolf should be a hi-top fade with a tall manicured crown');
    failures++;
  } else {
    note('✓', 'index.html wolf is a hi-top fade with a tall manicured crown');
  }
}

{
  ...
  if (!src.includes("sculptHair(`mohawkQ${r}`") || !src.includes("mullet: 'Mohawk'")
    ...
    note('✗', 'index.html — mullet slot should be a mohawk crest with shaved sides');
    failures++;
  } else {
    note('✓', 'index.html mullet slot is a mohawk crest with shaved sides');
  }
}

{
  ...
  if (!src.includes("geoCache(`layersD${r}`") || !src.includes('tucked behind the ears')
    ...
    note('✗', 'index.html — long layers should have a centre part tucked behind the ears');
    failures++;
  } else {
    note('✓', 'index.html long layers are a centre part tucked behind the ears');
  }
}

{
  ...
  if (!src.includes('longHair') || !src.includes("'longmiddle'") || !src.includes("'longwavy'")) {
    note('✗', 'index.html — long hair should offer bangs, middle part, side part, and wavy');
    failures++;
    ...
    note('✗', 'index.html — long bangs should part to the side with an open forehead');
    failures++;
  } else {
    note('✓', 'index.html long hair styles include part and wave variants');
  }
}

{
  ...
  if (!src.includes("from '/lib/profanity.js'") || !src.includes('publicNameError') || !src.includes('formPublicError')) {
    note('✗', 'index.html — names should be checked for offensive language');
    failures++;
  } else {
    note('✓', 'index.html blocks offensive names before save');
  }
}
...

{
  ...
  if (!wired) {
    note('✗', 'index.html — style review should cover tops and the other catalogues');
    failures++;
  } else if (!hairTurn) {
    note('✗', 'index.html — hair review should lay out front / side / back in a row');
    failures++;
  } else {
    note('✓', 'index.html has a style review page for tops, hats, hair, and the rest');
  }
}

{
  ...
  if (!src.includes('for="miiEmail">Email</label>')) {
    note('✗', 'index.html — email field should just say Email');
    failures++;
  } else if (src.includes('where your badge goes')) {
    note('✗', 'index.html — email label still has the long badge copy');
    failures++;
  } else if (!/max-width:\s*50%/.test(src) || !src.includes("id=\"camNameRow\"")) {
    note('✗', 'index.html — name and email inputs should be half width');
    failures++;
  } else {
    note('✓', 'index.html puts a half-width Email field under Name');
  }
}

{
  ...
  const wired = ["id: 'argyle'", "label: 'Argyle sweater'", "case 'argyle': {",
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — argyle sweater needs wrap-aware diamonds and dashed overlays');
    failures++;
  } else if (/CELINE|Triomphe|Celine/i.test(src)) {
    note('✗', 'index.html — argyle must not reprint a licensed house mark');
    failures++;
  } else {
    note('✓', 'index.html offers an argyle knit sweater');
  }
}

{
  ...
  if (!wired || !renamed) {
    note('✗', 'index.html — Mustache and Goatee rename plus a real Full Beard option');
    failures++;
  } else {
    note('✓', 'index.html has Mustache and Goatee plus a full lower-face beard');
  }
}

{
  ...
  const wired = ["id: 'tribal'", "label: 'Tribal'", 'function inkTribalSpiral',
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — tribal ink needs jaw painters and scaled face motifs on hands');
    failures++;
  } else {
    note('✓', 'index.html offers tribal tattoos on the jaw and hands');
  }
}

{
  ...
  if (!wired || !noHandsBoth) {
    note('✗', 'index.html — Traditional ink should replace Hands/Jaw/Both with hand+shoulder flash');
    failures++;
  } else {
    note('✓', 'index.html offers Traditional ink on hands and exposed shoulders');
  }
}

{
  ...
  if (!wired) {
    note('✗', 'index.html — hipster ink needs a watch, a candle, and under-eye marks');
    failures++;
  } else if (/LOVED|1 Cor|fillText/.test(hipster) || hipster.includes('777')) {
    note('✗', 'index.html — hipster ink must not reprint words, verses, or dates');
    failures++;
  } else {
    note('✓', 'index.html offers hipster tattoos on the face and hands');
  }
}

{
  ...
  const wired = ["id: 'minimal'", "label: 'Minimal'", 'function inkTwinTriangles',
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — minimal ink needs stacked triangles on hands and shoulders');
    failures++;
  } else {
    note('✓', 'index.html offers minimal tattoos on hands and exposed shoulders');
  }
}

{
  try {
    ...
    if (!sources.includes('/admin')) {
      ...
      failures++;
    } else if (!sources.includes('/') || !sources.includes('/mii')) {
      note('✗', 'vercel.json — / and /mii must be no-store so phones do not keep a stale office');
      failures++;
    } else {
      const home = (cfg.headers || []).find((h) => h && h.source === '/');
      const keys = (home.headers || []).map((h) => h && h.key);
      if (!keys.includes('Vercel-CDN-Cache-Control') && !keys.includes('CDN-Cache-Control')) {
        note('✗', 'vercel.json — / needs a CDN no-store header (Cache-Control alone still HITs)');
        failures++;
      } else {
        note('✓', 'vercel.json parses and pins The Office off the CDN');
      }
    }
  } catch (e) {
...

{
  ...
  if (!src.includes("sculptHair(`flowG${r}`") || !src.includes('Natural ear cutouts and a raised nape')
    ...
    note('✗', 'index.html — flow should trim ears/nape and keep even flowy texture');
    failures++;
  } else {
    note('✓', 'index.html flow trims ears/nape with even flowy texture');
  }
}

{
  ...
  if (!src.includes("sculptHair(`spkO${r}`")
    ...
    note('✗', 'index.html — spiky should use stubble sides/back with chunky crown spikes');
    failures++;
  } else {
    note('✓', 'index.html spiky has stubble sides/back with chunky crown spikes');
  }
}

{
  const src = readFileSync(join(ROOT, 'index.html'), 'utf8');
  if (!src.includes("sculptHair(`buzzAB${r}`")
      || src.includes("sculptHair(`buzzY${r}`") || !src.includes('stubble underlayer')
      ...
      || !src.includes('stubHairP')
      || !src.includes('yn=0 is the crown')
      || !src.includes('applyStubbleRimFade')
      || !src.includes('const step = 1.50')
    ...
    note('✗', 'index.html — buzz should be even stubble with a straight-across hairline');
    failures++;
  } else {
    note('✓', 'index.html buzz is even stubble with a straight-across hairline');
  }
}

{
  const src = readFileSync(join(ROOT, 'index.html'), 'utf8');
  if (!src.includes("sculptHair(`crewAC${r}`")
      || src.includes("sculptHair(`crewAB${r}`") || !src.includes('Military crown pad')
      ...
      || !src.includes('opaque DoubleSide DNA shell')
      || !src.includes("sculptHair(`fadeBandB${level}_${r}`")
      || src.includes("sculptHair(`crewAA${r}`")
      || src.includes("sculptHair(`crewZ${r}`")
      || src.includes("sculptHair(`crewY${r}`")
      || src.includes("sculptHair(`crewX${r}`")
      || src.includes("sculptHair(`crewR${r}`")
      || src.includes("sculptHair(`crewQ${r}`")
    ...
    note('✗', 'index.html — crew should have stubble sides and a longer shaved top');
    failures++;
  } else {
    note('✓', 'index.html crew has stubble sides and a longer shaved top');
  }
}

{
  const src = readFileSync(join(ROOT, 'index.html'), 'utf8');
  if (!src.includes("sculptHair(`fcropAA${r}`")
      || src.includes("sculptHair(`fcropZ${r}`") || !src.includes('gradual thickness fade into stubble')
      || !src.includes("st !== 'crop'")
      || !src.includes('lineupHemY')
      || !src.includes('fadeIntoStubble')
      || !src.includes('putStubbleFadeStack')
      || !src.includes("sculptHair(`fadeBandB${level}_${r}`")
      || src.includes("sculptHair(`fcropY${r}`")
      || src.includes("sculptHair(`fcropX${r}`")
      || src.includes("sculptHair(`fcropW${r}`")
      || src.includes("sculptHair(`fcropV${r}`")
      || src.includes("sculptHair(`fcropS${r}`")
      || src.includes("sculptHair(`fcropR${r}`")
    ...
    note('✗', 'index.html — french crop should fade on the sides with forward bangs');
    failures++;
  } else {
    note('✓', 'index.html french crop fades on the sides with forward bangs');
  }
}

{
  ...
  if (!src.includes("sculptHair(`pixG${r}`") || !src.includes('Dramatic side-V pixie')
    ...
    note('✗', 'index.html — pixie should have a dramatic side-V and chunky raised nape');
    failures++;
  } else {
    note('✓', 'index.html pixie has a dramatic side-V and chunky raised nape');
  }
}

{
  ...
  if (!src.includes("sculptHair(`quiffB${r}`") || !src.includes('G.quiff(1.05)')
    ...
    note('✗', 'index.html — quiff should have straight bangs, ears out, and a tapered nape');
    failures++;
  } else {
    note('✓', 'index.html quiff has straight bangs, ears out, and a tapered nape');
  }
}

{
  ...
  if (!src.includes("sculptHair(`pompP${r}`")
    ...
    note('✗', 'index.html — pompadour should be a continuous front→crown sweep that fades on the sides');
    failures++;
  } else {
    note('✓', 'index.html pompadour is a continuous front→crown sweep that fades on the sides');
  }
}

{
  ...
  if (!src.includes("sculptHair(`curtF${r}`") || !src.includes('G.curtains(1.05)')
    ...
    note('✗', 'index.html — curtains should be mid-length with big smooth chunks, ears out, and a raised nape');
    failures++;
  } else {
    note('✓', 'index.html curtains is mid-length with big smooth chunks, ears out, and a raised nape');
  }
}

{
  ...
  if (!src.includes("sculptHair(`slickB${r}`") || !src.includes('G.slickBack(1.05)')
    ...
    note('✗', 'index.html — slick back should have straight bangs, ears out, and a high nape fade');
    failures++;
  } else {
    note('✓', 'index.html slick back has straight bangs, ears out, and a high nape fade');
  }
}

{
  const src = readFileSync(join(ROOT, 'index.html'), 'utf8');
  if (!src.includes("sculptHair(`swoopAA${r}`")
      || src.includes("sculptHair(`swoopZ${r}`") || src.includes("sculptHair(`swoopY${r}`")
      || src.includes("sculptHair(`swoopX${r}`") || src.includes("sculptHair(`swoopW${r}`")
      || src.includes("sculptHair(`swoopV${r}`") || src.includes("sculptHair(`swoopU${r}`")
      || src.includes("sculptHair(`swoopT${r}`") || src.includes("sculptHair(`swoopS${r}`")
      || src.includes("sculptHair(`swoopR${r}`")
      || src.includes("sculptHair(`swoopO${r}`")
      ...
      || !src.includes('lineupHemY')
      || !src.includes('fadeIntoStubble')
      || !src.includes('Opaque DNA-color shell')
      || !src.includes('applyStubbleRimFade')
      || !src.includes('putStubbleFadeStack')
      || !src.includes("sculptHair(`fadeBandA${level}_${r}`")
    ...
    note('✗', 'index.html — swoop should be one solid left→right top mass without tuft gaps');
    failures++;
  } else {
    note('✓', 'index.html swoop is one solid left→right top mass without tuft gaps');
  }
}

{
  ...
  if (!src.includes("sculptHair(`shcurlD${r}`") || !src.includes("'shortcurl'")
    ...
    note('✗', 'index.html — short curls should be a curly crown with shaved sides, not two bumps');
    failures++;
  } else {
    note('✓', 'index.html short curls are a curly crown with shaved sides');
  }
}

{
  ...
  if (!src.includes("sculptHair(`bedC${r}`") || !src.includes('G.bedHead(1.06)')
    ...
    note('✗', 'index.html — side part should be a short shaggy bed head with frosted tips');
    failures++;
  } else {
    note('✓', 'index.html side part is a short shaggy bed head with frosted tips');
  }
}

{
  ...
  if (!fn || !/\n  return out;\n}\n\nfunction buildMii\(/.test(fn[0])) {
    note('✗', 'index.html — buildHair must return the hair meshes or The Office stays bald');
    failures++;
  } else {
    note('✓', 'index.html buildHair returns its meshes');
  }
}

{
  ...
  if (!src.includes("sculptHair(`sweptB${r}`") || !src.includes('G.sweptHair(1.05)')
    ...
    note('✗', 'index.html — swept should have a high side part, ear notches, and flowing clumps');
    failures++;
  } else {
    note('✓', 'index.html swept has a high side part, ear notches, and flowing clumps');
  }
}

{
  ...
  if (!src.includes("sculptHair(`wavD${r}`") || !src.includes('G.waves360(1.04)')
    ...
    note('✗', 'index.html — waves should be a long ponytail with a natural hairline');
    failures++;
  } else {
    note('✓', 'index.html waves is a long ponytail with a natural hairline');
  }
}

{
  ...
  if (!src.includes('function thinHairline') || !src.includes('function partGroove')
    ...
    note('✗', 'index.html — haircuts should use a curved hairline and a real part, not a hat brim');
    failures++;
  } else {
    note('✓', 'index.html haircuts use a curved hairline and a real part');
  }
}

{
  ...
  if (src.includes("sculptHair(`fadeY${r}`") || src.includes("st === 'fade'")
    ...
    note('✗', 'index.html — the fade helmet should be removed from the picker');
    failures++;
  } else {
    note('✓', 'index.html drops the fade helmet from the picker');
  }
}

{
  ...
  if (!src.includes("st === 'pigtails'")
    ...
    note('✗', 'index.html — pigtails should hang off the back of the head');
    failures++;
  } else {
    note('✓', 'index.html pigtails hang off the back of the head');
  }
}

{
  ...
  if (!src.includes('G.highPonyScalp(1.05)') || !src.includes('G.smoothPony()')
    ...
    note('✗', 'index.html — high ponytail should be one smooth tail with a curved nape');
    failures++;
  } else {
    note('✓', 'index.html high ponytail is one smooth tail with a curved nape');
  }
}

{
  ...
  if (!src.includes("sculptHair(`gathG${r}`") || !src.includes("sculptHair(`bunAK${r}`")
    ...
    note('✗', 'index.html — bun / pony / spacebuns should use a gathered scalp, not a hat-brim cap');
    failures++;
  } else {
    note('✓', 'index.html bun / pony / spacebuns use a gathered scalp with open forehead');
  }
}

{
  ...
  if (!noCapPicker || !noCapHat || !aliased) {
    note('✗', 'index.html — Cap should be removed from the hat catalogue and alias to flatbrim');
    failures++;
  } else {
    note('✓', 'index.html Cap is parked; old DNA aliases to flat brim');
  }
}

{
  ...
  if (!src.includes("geoCache('bktbrim4'") || !src.includes('G.bucketCrown(')
    ...
    note('✗', 'index.html — bucket should be a floppy canvas fisherman with a front patch');
    failures++;
  } else {
    note('✓', 'index.html bucket is a floppy canvas fisherman with a front patch');
  }
}

{
  ...
  if (!hatPicker) {
    note('✗', 'index.html — colour pickers need a Hat chip bound to hatColor');
    failures++;
  } else if (!oneBandana || !hatsOne || !labelled) {
    note('✗', 'index.html — bandana should be one hat, recolored from the Hat chip');
    failures++;
  } else if (!aliased) {
    note('✗', 'index.html — saved blue/green bandanas must alias onto bandana');
    failures++;
  } else {
    note('✓', 'index.html has a Hat colour chip and a single bandana');
  }
}

{
  ...
  const wired = ["id: 'techvest'", "label: 'Tech vest'", "kind: 'techvest'",
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — tech vest needs a black deep-V over a blue button-up');
    failures++;
  } else {
    note('✓', 'index.html offers a black tech vest with a deep V over a blue shirt');
  }
}

{
  ...
  if (!wired || !inCatalogue) {
    note('✗', 'index.html — vintage tee needs a raglan cut and an original office poster in Tops');
    failures++;
  } else if (licensed) {
    note('✗', 'index.html — vintage tee must not reprint a licensed tour shirt');
    failures++;
  } else {
    note('✓', 'index.html offers a raglan vintage tee with an original office print');
  }
}

{
  ...
  const wired = ["id: 'cutoff'", "label: 'Cutoff'", "kind: 'cutoff'",
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — cutoff needs a crop hem with skin midriff');
    failures++;
  } else {
    note('✓', 'index.html offers a cutoff crop with skin midriff');
  }
}

{
  ...
  const wired = ["id: 'bubble'", "label: 'Bubble jacket'", "kind: 'bubble'",
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — bubble jacket needs quilted baffles, a fur hood and puffy sleeves');
    failures++;
  } else {
    note('✓', 'index.html offers a quilted bubble jacket with a fur-trimmed hood');
  }
}

{
  ...
  if (!hooded || !folded) {
    note('✗', 'index.html — hooded tops need a small folded rest-hood on the upper back');
    failures++;
  } else {
    note('✓', 'index.html hooded tops use a folded rest-hood pouch with a collar rim');
  }
}

{
  ...
  if (!wired || frayed) {
    note('✗', 'index.html — tank top needs a V-neck, medium straps and a clean hem');
    failures++;
  } else {
    note('✓', 'index.html offers a bodycon tank with a V-neck');
  }
}

{
  ...
  const wired = ["id: 'protector'", "label: 'Pocket protector'", "kind: 'protector'",
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — pocket protector shirt needs pens sticking out of the plastic');
    failures++;
  } else {
    note('✓', 'index.html offers a button-up with a pocket protector');
  }
}

{
  ...
  const wired = ["id: 'boxhoodie'", "label: 'Box hoodie'", "kind: 'boxhoodie'",
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — box hoodie needs a red chest box with our own word');
    failures++;
  } else if (/Supreme/i.test(src)) {
    note('✗', 'index.html — box hoodie must not reprint a licensed box logo');
    failures++;
  } else {
    note('✓', 'index.html offers a box hoodie with a Co-Working chest mark');
  }
}

{
  ...
  if (!wired || !inCatalogue || !squeezed) {
    note('✗', 'index.html — Co- pocket tee needs an inward pocket stamp with X squeezed for sphere UVs');
    failures++;
  } else {
    note('✓', 'index.html offers a Co- pocket tee with a narrowed chest stamp');
  }
}

{
  ...
  const wired = ['function installPlazaProps', 'userData.plazaItem',
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — The Office needs clay office props with plazaItem tags');
    failures++;
  } else {
    note('✓', 'index.html installs clay plaza props (plants, lamp, coffee, cooler, desk, arcade)');
  }
}

{
  ...
  const wired = ['const PlazaProps', 'PlazaProps.boot()', 'PlazaProps.syncTip',
    ...
    .every((s) => src.includes(s));
  if (!wired) {
    note('✗', 'index.html — plaza props need hover titles via PlazaProps + #plazaTip');
    failures++;
  } else {
    note('✓', 'index.html shows hover titles on office props without blocking Co-Workers');
  }
}
